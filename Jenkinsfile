pipeline {
    ...
    environment {
        SERVICE_NAME          = 'greengrub-food-service'
                ...
                withCredentials([usernamePassword(credentialsId: 'Git_Cred', usernameVariable: 'GH_USER', passwordVariable: 'GH_TOKEN')]) {
                    ...
                    withSonarQubeEnv("${SONAR_SERVER_NAME}") {
                        sh """
                            ...
                            -Dsonar.projectKey=greengrub-food-service \
                            -Dsonar.projectName="GreenGrub - Food Service" \
        ...
        stage('Deploy to Kubernetes') {
            steps {
                withKubeConfig([credentialsId: "${K8S_CREDENTIALS_ID}", serverUrl: "${K8S_API_SERVER}"]) {
                    ...
                    sh "kubectl rollout status deployment/food-service -n services --timeout=780s"
                }
            }
        }
    }
    ...
    post {
        ...
        success {
            echo 'Food Service pipeline completed successfully.'
        }
        failure {
            echo 'Food Service pipeline failed.'
        }
    }
}
